  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const uData = uRes.data?.data?.items
        const tData = tRes.data?.data?.items
        setUsers((Array.isArray(uData) ? uData : uData?.data?.items ?? []).filter(u => u.global_role === 'client'))
        setProjectTypes(Array.isArray(tData) ? tData : tData?.data?.items ?? [])
      } catch {
