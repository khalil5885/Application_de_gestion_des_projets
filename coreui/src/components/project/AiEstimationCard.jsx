...
const AiEstimationCard = ({ project, onRecalculated }) => {
  ...
  const handleRecalculate = async () => {
    ...
    setError(null)
    try {
      const response = await api.post(`/api/admin/projects/${project.id}/estimation/recalculate`)
    ...
    <CCard className="border-0 shadow-sm mb-4">
      <CCardBody>
        ...
        <CRow className="g-3">
          ...
          <CCol md={4}>
            <div className="rounded-3 p-3 h-100" style={{ background: 'var(--cui-secondary-bg)' }}>
              <div className="small text-body-secondary mb-1">AI comment</div>
