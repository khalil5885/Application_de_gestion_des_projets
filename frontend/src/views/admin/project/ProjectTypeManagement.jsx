  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editing) {
        ...
        onSaved(res.data.data?.items, 'updated')
      } else {
        const res = await api.post('/api/admin/project-types', form)
        onSaved(res.data.data?.items, 'created')
      }
      onClose()
    } catch (err) {
...
const ProjectTypeManagement = () => {
  ...
  api.get('/api/admin/project-types')
    .then(res => {
      ...
      // New shape: { items: [...], meta: {...} }
      if (Array.isArray(payload?.items)) {
        setTypes(payload.items)
        setMeta(payload.meta ?? null)
      // Old paginated shape: { data: [...] }
      ...
      // Plain array fallback
        ...
        setTypes([])   // always set an array, never undefined/object
      }
    })
