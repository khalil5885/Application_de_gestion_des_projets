...
import api from '../../../api'
import CreateProjectTypeModal from '../../../components/project/CreateProjectTypeModal'
  ...
  const handleSave = async () => {
    ...
    try {
      await onSave(template.id, {
        ...
        order:            form.order !== '' ? parseInt(form.order) : template.order,
      })
      setEditing(false)
    ...
    return (
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          <button onClick={() => { setEditing(false); setForm({ ...template }) }} style={ghostBtn}>
            Cancel
          </button>
...
const NewTemplateForm = ({ projectTypeId, nextOrder, onCreated }) => {
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
        <button onClick={() => { setOpen(false); setError(null) }} style={ghostBtn}>Cancel</button>
      ...
      <div style={{
        ...
        <div style={{
          ...
          {loading ? (
            ...
            <div style={{ textAlign: 'center', paddingTop: 40, color: '#ef4444', fontSize: 13 }}>{error}</div>
          ) : templates.length === 0 ? (
            <div style={{ textAlign: 'center', paddingTop: 40, color: 'var(--cui-secondary-color)', fontSize: 13 }}>
              <CIcon icon={cilTask} style={{ width: 32, height: 32, opacity: 0.25, display: 'block', margin: '0 auto 12px' }} />
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <span style={{
          ...
          {projectType.task_templates?.length ?? projectType.task_templates_count ?? 0} templates · click to manage
        </span>
...
// ─── EditProjectTypeForm (inline, for editing existing types) ─────────────────

const EditProjectTypeForm = ({ initial, onSave, onCancel }) => {
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
        ...
        <button onClick={handleSubmit} disabled={saving || !form.name.trim()} style={primaryBtn}>
          {saving
            ? <CSpinner size="sm" />
            : <><CIcon icon={cilCheck} style={{ width: 13 }} /> Save Changes</>
          }
        </button>
      </div>
    </div>
...
const ProjectTypesPage = () => {
  ...
  const [error, setError]           = useState(null)
  const [creating, setCreating]     = useState(false)   // opens CreateProjectTypeModal
  ...
  // Called by CreateProjectTypeModal on submit
  ...
  const handleDelete = async (id) => {
    ...
    if (activeType?.id === id) setActiveType(null)
    fetchTypes()
  }
    ...
    <div style={{ padding: '32px 28px', maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 28 }}>
        ...
        <button
          ...
          style={{ ...primaryBtn, padding: '10px 20px', fontSize: 13 }}
        >
      ...
      {/* ── Create modal (new, extracted component) ── */}
      {creating && (
