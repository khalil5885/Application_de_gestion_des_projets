...
const EmployeeWorkloadDetail = lazy(() => import('../../views/admin/workload/EmployeeWorkloadDetail'))
...
const UserRow = ({ user, idx, onDelete, onView, onWorkload }) => {
...
const UserList = ({ refreshKey }) => {
  ...
  return (
    <>
            ...
            <AnimatePresence mode="popLayout">
              {visibleUsers.map((user, idx) => (
                <UserRow
                  ...
                  onWorkload={(user) => { setWorkloadUser(user); setShowWorkload(true); }}
                />
              ))}
            </AnimatePresence>
          )}
      ...
      <CModal visible={showWorkload} onClose={() => setShowWorkload(false)} size="lg">
        <CModalHeader>
          <CModalTitle>Employee Workload</CModalTitle>
        </CModalHeader>
        <CModalBody>
          <Suspense fallback={<CSpinner />}>
            {workloadUser && <EmployeeWorkloadDetail user={workloadUser} />}
          </Suspense>
        </CModalBody>
        <CModalFooter>
          <CButton color="secondary" onClick={() => setShowWorkload(false)}>Close</CButton>
        </CModalFooter>
      </CModal>
    </>
  )
